    ...
    var BaseSprintViewModel = (function (_super) {
        ...
        function BaseSprintViewModel() {
            ...
            this.sprintId.subscribe(function () {
                jQuery.cookie('sprint-' + _this.currentTeam().TeamId, _this.sprintId(), { expires: 365 });
            });
            ...
            jQuery.ajax({
                ...
                success: function (rawSprints) {
                    ...
                    if (selectedSprintId > 0) {
                        ...
                        _this.SwitchSprintById(parseInt(jQuery.cookie('sprint-' + _this.currentTeam().TeamId)));
                    }
                    else {
            ...
            if (typeof this.sprintId() === 'number') {
                jQuery.ajax({
                    ...
                    success: function (data) {
                        ...
                        for (var ii = 0; ii < data.length; ii++) {
                            var story = data[ii];
                            var newStory = new ScrumThing.Story(story.StoryId, story.Title, story.StoryText, story.Notes, story.StoryPoints, story.Ordinal, story.IsReachGoal, story.StoryTags);
